...
{
    /// <summary>
    /// Provides methods that are used by the common language runtime (CLR) to notify a code profiler when the events to which the profiler has subscribed occur.<para/>
    /// The ICorProfilerCallback2 interface is an extension of the <see cref="ICorProfilerCallback"/> interface. That is, it provides new callbacks introduced in .NET Framework version 2.0.
    /// </summary>
    /// <remarks>
    /// The CLR calls a method in the ICorProfilerCallback (or ICorProfilerCallback2) interface to notify the profiler
    /// when an event, to which the profiler had subscribed, occurs. This is the primary callback interface through which
    /// the CLR communicates with the code profiler. A code profiler must implement the methods of the ICorProfilerCallback
    /// interface. For the .NET Framework 2.0 and later versions, the profiler must also implement the ICorProfilerCallback2
    /// methods. Each method implementation must return an HRESULT having a value of S_OK on success or E_FAIL on failure.
    /// Currently, the CLR ignores the HRESULT that is returned by each callback except <see cref="ICorProfilerCallback.ObjectReferences"/>.
    /// A code profiler must register in the Microsoft Windows registry, its COM object that implements the ICorProfilerCallback
    /// and ICorProfilerCallback2 interfaces. A code profiler subscribes to the events for which it wants to receive notification
    /// by calling <see cref="ICorProfilerInfo.SetEventMask"/>. This is usually done in the profiler's implementation of
    /// <see cref="ICorProfilerCallback.Initialize"/>. The profiler is then able to receive notification from the runtime
    /// when an event is about to occur or has just occurred in an executing runtime process.
    /// </remarks>
...
#endif
    public partial interface ICorProfilerCallback2 : ICorProfilerCallback
    {
#if !GENERATED_MARSHALLING
        /// <summary>

namespace ClrDebug.DbgEng
{
    ...
    public class SvcStackFrameUnwinderTransition : ComObject<ISvcStackFrameUnwinderTransition>
    {
        ...
        /// The implementation can either leave the _Inout_ arguments alone and return UnwinderTransition none in the 'pTransitionKind' argument (indicating that no transition should occur)
        /// or it can adjust the _Inout_ arguments and indicate that it would like control of the unwind with specific behaviors as indicated by the 'pTransitionKind' argument.<para/>
        ...
        /// <summary>
        /// Given the information that would normally be passed to UnwindFrame, this asks the current unwinder whether it would like to stop "taking over" unwinding at a particular
        /// point and give the unwinding to another unwinder.<para/>
        /// The unwinder should have previously taken over via RequestsEntryTransition. The behaviors of the transition kind are identical to RequestsEntryTransition with the exception
        /// that the yes/no is opposite (UnwinderTransitionNone indicates continuation of use; others requests that the aggregate unwinder stop using the called unwinder).
        /// </summary>
        ...
        /// <summary>
        /// Given the information that would normally be passed to UnwindFrame after the *CURRENT* unwinder has reached the end of the stack, this asks an alternate unwinder if it
        /// would like to "take over" unwinding at this particular point.<para/>
        ...
        /// If multiple stack unwinders in an aggregate container indicate that they want a terminal transition, the request is passed to the unwinder which returns a register context
        /// that has a stack pointer closest to the stack pointer of the unwinder which terminated.
        /// </summary>

namespace ClrDebug.DbgEng
{
    ...
    /// </summary>
    public class DebugHostTypeSignature : ComObject<IDebugHostTypeSignature>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="DebugHostTypeSignature"/> class.
        /// </summary>
        /// <param name="raw">The raw COM interface that should be contained in this object.</param>
        public DebugHostTypeSignature(IDebugHostTypeSignature raw) : base(raw)
        {
        }

        #region IDebugHostTypeSignature
        #region HashCode
        ...
        public IsMatchResult IsMatch(IDebugHostType type)
        {
            IsMatchResult result;
            TryIsMatch(type, out result).ThrowDbgEngNotOK();
        ...
        /// <returns>This method returns HRESULT that indicates success or failure.</returns>
        public HRESULT TryIsMatch(IDebugHostType type, out IsMatchResult result)
        {
            /*HRESULT IsMatch(
            ...
            HRESULT hr = Raw.IsMatch(type, out isMatch, out wildcardMatches);

            if (hr == HRESULT.S_OK)
                result = new IsMatchResult(isMatch, DebugHostSymbolEnumerator.New(wildcardMatches));
            else
                result = default(IsMatchResult);
        ...
        /// <returns>An indication of the relationship between the two signatures is returned here -- whether they are unrelated or identical, one is more or less specific than the other, or they are ambiguous.<para/>
        /// Such relationship is given by a member of the SignatureComparison enumeration.</returns>
        public SignatureComparison CompareAgainst(IDebugHostTypeSignature typeSignature)
        {
            SignatureComparison result;
            TryCompareAgainst(typeSignature, out result).ThrowDbgEngNotOK();
        ...
        /// <returns>This method returns HRESULT that indicates success or failure.</returns>
        public HRESULT TryCompareAgainst(IDebugHostTypeSignature typeSignature, out SignatureComparison result)
        {
            ...
            return Raw.CompareAgainst(typeSignature, out result);
        }
